...
- 아키텍처: `파이프라인 + 팬아웃/팬인` 복합 패턴
  ...
  [오케스트레이터 (이 스킬)]
      ...
      │
      └─ Phase 6b: Agent(web-publisher) ── topic 뷰 HTML + Deep Research 인덱스 + 정적 호스팅 배포
  ```
...
- **에이전트 분배**:
  ...
  - `web-publisher` (1개): topic 뷰 index.html + Deep Research `_search_index.json` 빌드 + `git push origin master` (정적 호스팅 자동 배포)
...
**핵심 원칙:**
1. **리뷰는 papers/{slug}/에만 존재** — topic 디렉토리에 리뷰 복사하지 않음
2. **리뷰 작성 전 _papers_index.json 검색** — 이미 리뷰가 있으면 topics 배열에 현재 주제만 추가
3. **topic 뷰의 index.html은 ../papers/{slug}/index.html로 링크**
4. **slug는 전역 고유** — {NNN}_{SafeTitle} 형식, NNN은 papers/ 전체에서 순번
5. **기존 topic 디렉토리(ai4s/ 등)에 있던 리뷰는 papers/로 마이그레이션** 필요
</Execution_Policy>
...
1. **Phase 0 - 파라미터 설정**
   ...
   | 모드 | 트리거 | 실행 Phase |
   |------|--------|-----------|
   | `full` (기본) | "논문 큐레이션", "최신 논문 찾아줘" | Phase 0→1→2→3→4→5→6→7 전체 |
   | `local` | "가지고 있는 논문으로", "--local" | Phase 0→(1~5 스킵)→6→7 |
   | `update` | "--update", "업데이트", "새 논문 반영" | Phase 0→1→2→3→4→5→(diff)→6(신규만)→6b(전체 재빌드)→7 |
   | `update-force` | "--update-force", "전체 재생성", "다시 만들어" | Phase 0→(소스 결정)→6(전체 재생성)→6b(전체 재빌드)→7 |
   | `local --update` | "--local --update", "--local 업데이트" | Phase 0→(Zotero diff만)→6(신규만)→6b(전체 재빌드)→7 |
   | `local --update-force` | "--local --update-force" | Phase 0→(Zotero 전체)→6(전체 재생성)→6b(전체 재빌드)→7 |

   `--local`, `--update`, `--update-force`는 **독립 플래그**로 조합 가능:

   | local | update | update-force | 웹 검색 | Zotero 등록 | diff 기준 | 리뷰 대상 | 기존 review.md | classification | 타임라인 |
   |:-----:|:------:|:------------:|:------:|:----------:|----------|----------|:-------------:|:-------------:|:--------:|
   | X | X | X | O | O | - | 전체 | 새로 생성 | 새로 생성 | 전체 재생성 |
   | O | X | X | X | X | - | Zotero 전체 | 새로 생성 | 새로 생성 | 전체 재생성 |
   | X | O | X | O | O | 기존 vs 검색결과 | 신규만 | **유지** | **기존 유지** | 변경 카테고리만 |
   | O | O | X | X | X | 기존 vs Zotero | 신규만 | **유지** | **기존 유지** | 변경 카테고리만 |
   | X | X | O | O | O | - | 전체 | **삭제 후 재생성** | 재생성 | 전체 재생성 |
   | O | X | O | X | X | - | Zotero 전체 | **삭제 후 재생성** | 재생성 | 전체 재생성 |

   **`--update` 핵심 원칙**:
   - `classifications[topic]`이 이미 있는 논문은 분류 스킵. 신규 논문만 분류.
   - 기존 분류를 절대 덮어쓰지 않음.
   - classification 기준(TOPIC_KEYWORDS)은 절대 변경하지 않음.

   **`--update-force` 핵심 원칙**: 기존 papers/{slug}/의 review.md, figures/, index.html, text.md를 **모두 삭제 후 재생성**. PDF부터 다시 파싱하여 완전히 새로운 리뷰를 생성. classification, timeline, category descriptions도 전부 재생성.
...
4. **Phase 6 - Paper Review 작성 및 정적 호스팅 배포**

   등록된 각 논문에 대해 리뷰를 생성하고, 토픽 인덱스와 Deep Research 검색 인덱스를 빌드한 뒤 정적 호스팅(GitHub Pages, Cloudflare Workers 등)에 `master` push로 배포한다.
   ...
   **핵심 원칙:**
   ...
   모든 `papers/*/review.md`에서 메타데이터를 추출하여 `papers/_papers_index.json` 재생성.
...
- `Agent(subagent_type="web-publisher")`: Phase 6b — **topic 뷰** HTML 빌드 + `build_search_index.py` (Deep Research RAG 인덱스) + 정적 호스팅 자동 배포 (Research Timeline + 정렬 기능 + DOI 링크 + Deep Research UI 포함)
...
- [ ] 인덱스 페이지 생성 (요약 + 핵심 그림 + 점수 + Deep Research UI)
- [ ] Deep Research 검색 인덱스 빌드 (`{topic}/_search_index.json`)
- [ ] 정적 호스팅 배포 (/{분야}_{YYMMDD}/) — `master` push 자동
- [ ] 배포 URL 확인
- [ ] 결과 보고 (등록 수, 리뷰 수, 인덱스 chunks 수, 배포 URL)
</Final_Checklist>
...
| `run_update_force.py` | PDF→review 배치 | Zotero API | `papers/{slug}/` |
| `build_papers_index.py` | 마스터 인덱스 + 스키마 마이그레이션 | `papers/*/review.md` | `_papers_index.json` (`classifications[topic]`) |
...
| `review_to_html.py` | review→HTML 변환 | `papers/*/review.md` | `papers/*/index.html` |
| `build_topic_index.py` | 인덱스 페이지 빌드 | 위 모든 파일 | `{topic}/index.html` |
| `prepare_deploy.py` | 배포 준비 (PNG→WebP, gitignore, push) | `papers/*/figures/*.png` | `.webp` + git push |
</Advanced>
